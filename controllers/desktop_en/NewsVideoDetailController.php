...
class NewsVideoDetailController extends CController {
    ...
    function __construct() {
        parent::__construct();
        $this->model(['News','Tag', 'TagNews', 'NewsRelated', 'NewsPaging', 'Video', 'SponsorTag'],null, true);
    ...
    function index($category = '', $slug = '', $paging = '') {
        ...
        $cacheKey = 'desktop_en_video_news_detail_' . $slug.'_'.$key_preview;
        ...
        if (!empty($mongo_data))
        {
            ...
            if ( (str_replace('!', '',  strtolower($news_detail['news_category_name'])) ) !== $category )
            {
        ...
        else
        {
            ...
            if ($_video[0])
            {
                ...
                //GET TAG
                $tags = cache('query_en_news_tags_'.$news_detail['news_id'], function () use($news_detail) {
               ...
               $cross = cache('query_en_news_crosslink_'.$news_detail['news_id'], function () use($news_detail) {
               ...
               if ($news_detail['has_paging'] == 1){
                  $paging_ = cache('query_en_berita_paging_'. $news_detail['news_id'] , function() use($news_detail) {
              ...
              {
                   $news_detail['news_image_intro_location_full']     = $news_detail['news_image_location']. '/200x100-'. $news_detail['news_image_potrait'];
              }else
              {
                   $news_detail['news_image_intro_location_full']     = $news_detail['news_image_location_full'];
              }
              ...
              if ($keyword_paging)
              {
                  $is_paging = cache('query_en_cek_content_paging_'.$keyword_paging. '_' . $news_detail['news_id'], function() use($keyword_paging,$news_detail)
                  {
              ...
              if ($news_detail['has_paging'] == 1)
              {
                  ...
                  if ($status_order== 0)
                  {
                      ...
                      $paging_list = cache('query_en_nav_top_paging_'.$keyword_paging. '_' .$news_detail['news_id'], function() use($keyword_paging, $news_detail, $limit, $offset)
                      {
                        return news_paging::where('news_paging_status','1')
                  ...
                  else
                  {
                      ...
                      // $paging_list =  news_paging::where('news_paging_status','1')
                        //       ->where('news_paging_news_id',$news_detail['news_id'])->skip($offset)->take($limit)->get()->toArray();
    ...
    if( !empty($news_detail['sponsor_tag']) )
    {
      ...
      $news_detail['reporter']['name']   = $this->view('desktop_en/box/_box_brand', ['brand' => $news_detail['sponsor_tag'], 'TE' => $TE ], true);
    ...
    foreach ($this->config['keyword_brand'] as $key => $value)
    {
        ...
        $brand['brand_url']                 = '#';
        $news_detail['reporter']['name']    = $this->view('desktop_en/box/_box_brand', ['brand' => $brand, 'TE' => $TE ], true);
        $news_detail['reporter_style_date'] = 'style="margin-top: 28px;"';
        $news_detail['reporter_status']     = 'brand';
        $init_brand = TRUE;
        }
    ...
    //eof brand image reporter
      ...
      if (isset($news_detail['news_crosslink']) && !empty($news_detail['news_crosslink'])) {
            ...
            foreach ($cross as $key=>$value)
            {
                ...
                {
                    $value['TE'] = $TE;
            $cross = $this->view('desktop_en/box/_news_crosslink',['cross' => $value],TRUE);
      ...
      $url_amp = $this->config['base_url']. 'amp/' . strtolower($news_detail['news_category_name']) . '/' . $news_detail['news_url'] . '.html';
        ...
        [
            ...
            'meta_alternate'    => $this->config['m_url_en'] . substr($_SERVER['REQUEST_URI'], strlen($this->config['rel_url'])),
            'iframe_kl'         => $this->config['www_url_en'] . substr($_SERVER['REQUEST_URI'], strlen($this->config['rel_url'])),
            'amphtml'           => isset($url_amp) ? $url_amp : $this->config['www_url_en'] ,
        ];
        ...
        [
            ...
            'news_more_stories' => $this->view('desktop_en/video/_news_more_stories', ['TE'=> $TE, 'related_news_list' => $news_detail['related_news_list'] ], TRUE),
            'trending'      => $this->view('desktop_en/box/right_trending', $this->_trending(7, $TE), TRUE),
            'collect_email'     => $this->view('desktop_en/box/_email', [], TRUE),
            'bottom_menu'       => $this->view('desktop_en/box/bottom_menu', ['TE' => $TE ], TRUE),
            'video'                => $this->view('desktop_en/box/promote_video', ['promote_video' => $this->get_video_sponsor($news_detail['news_sponsorship'], $init_brand) ], TRUE),
        ...
        if (empty($mongo_data) && ($this->config['json_news_detail'] === TRUE))
        {
            ...
            writeDataMongo($mongo_file, $news_detail, $this->config['mongo_prefix'] . "video");
        }

        $ret = $this->_render('desktop_en/video/detail_video', $data);
        $interval = WebCache::App()->get_config('cachetime_default');
        setCache($cacheKey, $ret, 1);
        return $ret ;
	...
	function more_stories($news_detail, $TE){
        $interval   = WebCache::App()->get_config('cachetime_default');
        $cacheKey = 'desktop_en_video_more_stories_' . MD5($news_detail['news_id']);
        ...
        $_v = cache('query_en_video_more_stories_desktop_'.$news_detail['news_id'], function() use( $news_detail, $total_related) {

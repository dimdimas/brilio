...
class NewsDetailController extends CController {
	 ...
	 function index( $category = '', $slug = '', $url_paging_news = '')
	 {
        ...
        {
            $cacheKey = 'dekstop_en_read_news_content_'.$slug.'_'.$key_preview;
        ...
        {
            $cacheKey = 'dekstop_en_read_news_content_'.$slug.'_'.$url_paging_news.'_'.$key_preview;
          ...
          if($news_detail['news_date_publish'] >= date('Y-m-d H:i:s') && $preview_flag == FALSE)
				  ...
				  if ($news_detail['has_paging'] == 1)
		      {
					  ...
					  if ($news_detail['has_paging'] == 1)
					  {
							...
							if ($status_order== 0)
							{
								 $paging_list = cache('query_en_nav_top_paging_'.$keyword_paging. '_' .$news_detail['news_id'], function() use($keyword_paging, $news_detail)
								 {
								 	return news_paging::where('news_paging_status','1')
    ...
    $news_detail['news_category_name'] = str_replace('!', '', $news_detail['news_category_name']);
		...
		[
			...
			'news_more_stories'    => $this->view('desktop_en/news/_news_more_stories', ['TE' => $TE ,'related_news_list' => $news_detail['related_news_list'] ], TRUE),
		...
		$ret = $this->_render('desktop_en/news/newsdetail', $data);
	...
	function more_stories($news_detail, $TE){
		$interval   = WebCache::App()->get_config('cachetime_default');
		$cacheKey = 'desktop_en_more_stories_' . MD5($news_detail['news_id']);
		...
		$_v = cache('query_en_more_stories_desktop_'.$news_detail['news_id'], function() use( $news_detail, $total_related) {

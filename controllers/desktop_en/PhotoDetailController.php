...
class PhotoDetailController extends CController {
    ...
    function index($category='', $url_keyword_news = '', $url_paging_news = '')
    {
        ...
        $cacheKey   = 'desktop_en_read_photo_content_' . $url_filter.'_'.$key_preview;
        ...
        {
            $news_detail = cache("dekstop_en_photo_query_more_stories_".$keyword_news, function () use ($keyword_news){
        ...
        {
            $data_visit_photos = cache("desktop_en_query_".$cacheKey."data_visit_photos", function () use($all_photos, $keyword_paging) {
        ...
        {
            $prev_photos = cache("desktop_en_query_".$cacheKey."prev_photos_visit", function () use ($visit_page_no, $get_photo_list) {
                if($visit_page_no == 1)
                {
                    return "";
                }
        ...
        {
            $next_photos = cache("desktop_en_query_".$cacheKey."next_photos_visit", function () use ($visit_page_no, $get_photo_list) {
        ...
        else
        {
            ...
            $get_photo_small = cache("query_en_desktop_".$cacheKey."_get_photo_".$start."_".$limit, function () use ($all_photos, $start, $limit){
        ...
        if( !empty($news_detail['sponsor_tag']) )
        {
            ...
            $news_detail['reporter']            = $this->view('desktop_en/box/_box_brand', ['brand' => $news_detail['sponsor_tag'], 'TE' => $TE ], true);
        ...
        foreach ($this->config['keyword_brand'] as $key => $value)
        {
            if ($key == $keyword_news )
            {
                ...
                $brand['brand_url']                 = '#';
                $news_detail['reporter']            = $this->view('desktop_en/box/_box_brand', ['brand' => $brand, 'TE' => $TE], true);
                $news_detail['reporter_style_date'] = 'style="margin-top: 28px;"';
                $news_detail['reporter_status']     = 'brand';
                $init_brand = TRUE;
            }
        ...
        //function crosslink
        $cross = cache('desktop_en_news_crosslink_'. $cache_news_id , function () use($news_detail) {
        ...
        [
            ...
            'list_related_news' => $this->view('desktop_en/photo/_news_more_stories', ['TE' => $TE , 'related_news_list' => $news_detail['related_news_list'] ], TRUE),
        ...
        $ret      = $this->_render('desktop_en/photo/photodetail', $data);
    ...
    function more_stories($news_detail, $TE){
        $interval   = WebCache::App()->get_config('cachetime_default');
        $cacheKey = 'desktop_en_photo_more_stories_' . $news_detail['news_id'];
        ...
        $_v = cache('query_en_photo_more_stories_dekstop_'.$news_detail['news_id'], function() use( $date_publish, $total_related) {

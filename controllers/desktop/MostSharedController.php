...
class MostSharedController extends CController {
    ...
    function index($thn = '', $bln = '', $tgl = '')
    {
        ...
        $most_shared_news = $this->news_most_shared($thn, $bln, $tgl);
        $news                = $most_shared_news;

        $og_image            = '';
        $og_image_secure     = '';
        $og_url              = '';
        if(!isset($most_shared_news['ERROR']))
        {
            $most_shared_news = $most_shared_news[0];
            $og_url          = $most_shared_news['OG_URL'];
            $sosmed          = $this->sosmed($most_shared_news['OG_URL']);
            $og_image        = 'http://cdn.klimg.com/newshub.id/'. substr($most_shared_news['NEWS_IMAGES'], strlen($this->config['klimg_url']));
            $og_image_secure = 'https://cdn.klimg.com/newshub.id/'. substr($most_shared_news['NEWS_IMAGES'], strlen($this->config['klimg_url']));
        }
        ...
        $meta =
        [
            'meta_title'         => 'Most Shared Articles',
            'meta_description'   => 'Kisah dan cerita kehidupan, gaya hidup, renungan, inspirasi, dan ilmu pengetahuan paling banyak dibagi',
            'og_url'             => $og_url,
            'og_image'           => $og_image,
            'og_image_secure'    => $og_image_secure,
            'expires'            => date(DATE_RFC1036),
            'meta_keywords'      => 'Kisah, dan, cerita, kehidupan, gaya, hidup, renungan, inspirasi, dan, ilmu, pengetahuan, paling, banyak, dibagi',
        ...
        [
            ...
            'EDITOR_PICKS'      => $this->view('desktop/box/right_editors_pick', $this->_editor_picks("desktop-editor-picks-most-shared", $TE, 7), TRUE),
            'JUST_UPDATE'       => $this->view('desktop/box/right_just_update', $this->_just_update("desktop-just-update-most-shared", $TE), TRUE),
            'CHECK_THIS_OUT'    => $this->view('/desktop/box/right_check_this_out', $this->_check_this_out($TE, 7), TRUE),
        ];

        $ret      = $this->_render('desktop/most-shared/index_most_shared', $data);
    ...
    function breadcrumb($thn, $bln, $tgl)
    {
        ...
        $cacheKey = 'desktop-breadcrumb-most_shared'.$tanggal.'-'.$thn;
    ...
    function menu_type_date($thn, $bln, $tgl)
    {
        ...
        $cacheKey = 'menu_type_date-most_shared'.$tanggal.'-'.$thn;
        if ($ret = checkCache($cacheKey))
        {
            return unserialize($ret);
        }
        ...
        if (empty($thn))
        {
            ...
            $class_week  = '';
            $link_today  = '#';
            $link_week   = $base_url . 'most-shared/week';
        }
        else
        {
            if ($thn != 'week')
            ...
            else
            {
                ...
                {
                    $class_today = '';
                    $link_today  = $base_url . 'most-shared/today';
                }
                if ($thn == 'week')
                {
    ...
    // HEADLINE
    function news_most_shared($thn, $bln, $tgl)
    {
        $interval = WebCache::App()->get_config('cachetime_short');
        ...
        $cacheKey = 'desktop-news_most_shared-'.$tanggal.'-'.$thn;
        if ($ret = checkCache($cacheKey))
        {
            return unserialize($ret);
        }
        ...
        {
            return newsPopular('share', $this->config['mongo_prefix'], strtotime($start_date), strtotime($end_date), 20);
    ...
    //SOSMET
    function sosmed($url = ''){
        $cacheKey = 'desktop_sosmed-most_shared'.  md5($url);
        if ($ret = checkCache($cacheKey))
        {
            return $ret;
        }

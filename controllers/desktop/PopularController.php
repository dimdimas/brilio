<?php
class PopularController extends CController {
    ...
    function index($thn = '', $bln = '', $tgl = '')
    {
        ...
        $cacheKey = 'desktop_index_most_popular_'.$tanggal.'-'.$thn;
        ...
        $most_commented_news = $this->news_most_commented($thn, $bln, $tgl);
        $news                = $most_commented_news;

        $og_image            = '';
        $og_image_secure     = '';
        $og_url              = '';
        if(!isset($most_commented_news['ERROR']))
        {
            $most_commented_news = $most_commented_news[0];
            $og_url          = $most_commented_news['OG_URL'];
            $sosmed          = $this->sosmed($most_commented_news['OG_URL']);
            $og_image        = 'http://cdn.klimg.com/newshub.id/'. substr($most_commented_news['NEWS_IMAGES'], strlen($this->config['klimg_url']));
            $og_image_secure = 'https://cdn.klimg.com/newshub.id/'. substr($most_commented_news['NEWS_IMAGES'], strlen($this->config['klimg_url']));
        }
        ...
        $meta =
        [
            'meta_title'         => 'Most Popular Articles',
            'meta_description'   => 'Kisah dan cerita kehidupan, gaya hidup, renungan, inspirasi, dan ilmu pengetahuan paling banyak dikomentari',
            'og_url'             => $og_url,
            'og_image'           => $og_image,
            'og_image_secure'    => $og_image_secure,
            'expires'            => date(DATE_RFC1036),
            'meta_keywords'      => 'Kisah, dan, cerita, kehidupan, gaya, hidup, renungan, inspirasi, dan, ilmu, pengetahuan, paling, banyak, dikomentari',
        ...
        [
            ...
            'EDITOR_PICKS'      => $this->view('desktop/box/right_editors_pick', $this->_editor_picks("desktop-editor-picks-popular_", $TE, 7), TRUE),
            'JUST_UPDATE'       => $this->view('desktop/box/right_just_update', $this->_just_update("desktop-just-update-popular_", $TE), TRUE),
            'CHECK_THIS_OUT'    => $this->view('/desktop/box/right_check_this_out', $this->_check_this_out($TE, 7), TRUE),
        ];

        $ret      = $this->_render('desktop/most-popular/index_popular', $data);
    ...
    function breadcrumb($thn, $bln, $tgl)
    {
        ...
        $cacheKey = 'desktop_breadcrumb_most_popular_'.$tanggal.'-'.$thn;
        if ($ret  = checkCache($cacheKey))
        {
    ...
    function menu_type_date($thn, $bln, $tgl)
    {
        ...
        $cacheKey = 'desktop_menu_type_date_most_popular_'.$tanggal.'-'.$thn;
        if ($ret = checkCache($cacheKey))
        {
            return unserialize($ret);
        }
        ...
        if (empty($thn))
        {
            ...
            $class_week  = '';
            $link_today  = '#';
            $link_week   = $base_url . 'popular/week';
        }
        else
        {
            if ($thn != 'week')
            ...
            else
            {
                ...
                {
                    $class_today = '';
                    $link_today  = $base_url . 'popular/today';
                }
                if ($thn == 'week')
                {
    ...
    function menu_per_date($thn, $bln, $tgl)
    {
        ...
        $cacheKey = 'desktop_menu_per_date_most_popular_'.$tanggal.'-'.$thn;
        if ($ret = checkCache($cacheKey))
        {
            return unserialize($ret);
        }
    ...
    // HEADLINE
    function news_most_commented($thn, $bln, $tgl)
    {
        ...
        $interval = WebCache::App()->get_config('cachetime_short');
        $cacheKey = 'desktop_most_popular_'.date("Y-m-d").'-'.$tanggal.'-'.$thn;
        ...
        $news = cache("desktop_query_most_popular_".$cacheKey, function () use ($start_date, $end_date)
        {
            return newsPopular('pageview', $this->config['mongo_prefix'], strtotime($start_date), strtotime($end_date), 20);
            ...
            foreach ($news as $data)
            {
                if($no == 1)
                {
                    ...
                    if (is_numeric($data['news_entry'])) {
                      $data_news['NEWS_ENTRY']    = $this->lib_date->indo( date('Y-m-d H:i:s',$data['news_entry'] )); //format 22 April 2015 13.30
                    }else{
                      $data_news['NEWS_ENTRY']    = $this->lib_date->indo($data['news_entry']); //format 22 April 2015 13.30
                    }
                    $data_news['NEWS_URL']      = $data['news_url_full'];
                    $data_news['OG_URL']        = $data['news_url_with_base'];
                    $data_news["TAGS_TITLE"]    = $data['tag_name'];
                    if (!empty($data['tag_url_full']))
                    {
    ...
    //SOSMET
    function sosmed($url = ''){
        $cacheKey = 'desktop_sosmed_most_popular_'.  md5($url);
        if ($ret = checkCache($cacheKey))
        {
            return $ret;
        }

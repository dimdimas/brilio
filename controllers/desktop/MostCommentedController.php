<?php
class MostCommentedController extends CController {
    ...
    function index($thn = '', $bln = '', $tgl = '')
    {
        ...
        $cacheKey = 'index-most_commented'.$tanggal.'-'.$thn;
        ...
        $meta =
        [
            'meta_title'         => 'Most Commented Articles',
        ...
        [
            ...
            'EDITOR_PICKS'      => $this->view('desktop/box/right_editors_pick', $this->_editor_picks("desktop-editor-picks-most-commented", $TE, 7), TRUE),
            'JUST_UPDATE'       => $this->view('desktop/box/right_just_update', $this->_just_update("desktop-just-update-most-commented", $TE), TRUE),
            'CHECK_THIS_OUT'    => $this->view('/desktop/box/right_check_this_out', $this->_check_this_out($TE, 7), TRUE),
        ];

        $ret      = $this->_render('desktop/most-commented/index_most_commented', $data);
    ...
    function breadcrumb($thn, $bln, $tgl)
    {
        ...
        $cacheKey = 'desktop-breadcrumb-most_commented'.$tanggal.'-'.$thn;
        if ($ret  = checkCache($cacheKey))
        {
    ...
    function menu_type_date($thn, $bln, $tgl)
    {
        ...
        $cacheKey = 'menu_type_date-most_commented'.$tanggal.'-'.$thn;
        if ($ret = checkCache($cacheKey))
        {
            return unserialize($ret);
        }
        ...
        if (empty($thn))
        {
            ...
            $class_week  = '';
            $link_today  = '#';
            $link_week   = $base_url . 'most-commented/week';
        }
        else
        {
            if ($thn != 'week')
            ...
            else
            {
                ...
                {
                    $class_today = '';
                    $link_today  = $base_url . 'most-commented/today';
                }
                if ($thn == 'week')
                {
    ...
    function menu_per_date($thn, $bln, $tgl)
    {
        ...
        $cacheKey = 'menu_per_date-most_commented'.$tanggal.'-'.$thn;
        if ($ret = checkCache($cacheKey))
        {
            return unserialize($ret);
        }
    ...
    function news_most_commented($thn, $bln, $tgl)
    {
        ...
        $cacheKey = 'desktop-news_most_commented-'.$tanggal.'-'.$thn;
        if ($ret = checkCache($cacheKey))
        {
            return unserialize($ret);
        }
        ...
        {
            return newsPopular('comment', $this->config['mongo_prefix'], strtotime($start_date), strtotime($end_date), 20);
    ...
    //SOSMET
    function sosmed($url = ''){
        $cacheKey = 'desktop_sosmed-most_commented'.  md5($url);
        if ($ret = checkCache($cacheKey))
        {
            return $ret;
        }

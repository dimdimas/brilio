...
class NewsVideoDetailController extends CController {
    ...
    function index($category = '', $slug = '', $paging = '') {
        ...
        $cacheKey = 'desktop_video_news_detail_' . $slug.'_'.$key_preview;
    ...
    //eof brand image reporter
      ...
      $url_amp = $this->config['base_url']. 'amp/' . strtolower($news_detail['news_category_name']) . '/' . $news_detail['news_url'] . '.html';
        ...
        [
            ...
            'news_more_stories' => $this->view('desktop/video/_news_more_stories', ['TE'=> $TE, 'related_news_list' => $news_detail['related_news_list'] ], TRUE),
        ...
        $ret = $this->_render('desktop/video/detail_video', $data);
        $interval = WebCache::App()->get_config('cachetime_default');
	...
	function more_stories($news_detail, $TE){
        $interval   = WebCache::App()->get_config('cachetime_default');
        $cacheKey = 'desktop_video_more_stories_' . MD5($news_detail['news_id']);
        ...
        $_v = cache('query_video_more_stories_desktop_'.$news_detail['news_id'], function() use( $news_detail, $total_related) {

...
class NewsVideoDetailController extends CController {
    ...
    function index($category = '', $slug = '', $paging = '') {
        ...
        $cacheKey = 'mobile_video_news_detail_' . $slug.'_'.$key_preview;
      ...
      if (isset($news_detail['news_crosslink']) && !empty($news_detail['news_crosslink'])) {
            ...
            foreach ($cross as $key=>$value)
            {
                ...
                {
                    $cross = $this->view('mobile/video/_crosslink',['cross' => $value],TRUE);
      ...
      $data_breadcumb['category_url']     = strtolower($news_detail['news_category_url']);
        ...
        [
            ...
            'news_related'  => $this->view('mobile/detail/_news_related', ['related_news_list' => array_slice($news_detail['related_news_list'], 0, 3)], TRUE),
        ...
        $ret = $this->_mobile_render('mobile/video/newsvideodetail', $data);
        $interval = WebCache::App()->get_config('cachetime_default');
   ...
   function more_stories($news_detail, $TE){
   	...
   	$interval   = WebCache::App()->get_config('cachetime_default');
   	$cacheKey = 'mobile_video_more_stories_' . MD5($news_detail['news_id']);

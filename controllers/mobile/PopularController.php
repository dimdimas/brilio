...
class PopularController extends CController {
    ...
    function index($thn = '', $bln = '', $tgl = '') {
        ...
        $cacheKey = 'index-popular-mobile'.$tanggal.'-'.$thn;
        ...
        $ret =  $this->_mobile_render('mobile/popular/index_popular', $data);
    ...
    function menu_type_date($thn, $bln, $tgl) {
        ...
        $cacheKey = 'menu_type_date_popular_mobile_'.$tanggal.'-'.$thn;
        if ($ret = checkCache($cacheKey)) return $ret;
    ...
    function menu_per_date($thn, $bln, $tgl) {
        ...
        $cacheKey = 'menu_per_date_popular_mobile_'.$tanggal.'-'.$thn;
        if ($ret = checkCache($cacheKey)) return $ret;
    ...
    function popular($thn, $bln, $tgl) {
        ...
        $interval = WebCache::App()->get_config('cachetime_short');
        $cacheKey = 'mobile_most_popular_'.date("Y-m-d").'-'.$thn;
        ...
        $news = cache("mobile_query_popular".$cacheKey, function () use ($start_date, $end_date)
        {
    ...
    //SOSMET
    function sosmed($url = ''){
        $cacheKey = 'mobile_sosmed-popular'.  md5($url);
        if ($ret = checkCache($cacheKey))
        {
            return $ret;
        }
        ...
        $ret = $this->view('mobile/box/_sosmed', $sosmed_data, true);
        $interval = WebCache::App()->get_config('cachetime_short');
        setCache($cacheKey, $ret, $interval);
        return $ret;
    }

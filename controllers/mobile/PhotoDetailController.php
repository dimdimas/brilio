...
class PhotoDetailController extends CController {
    ...
    function index($category= '', $url_keyword_news = '', $url_paging_news = '')
    {
        ...
        $cacheKey   = 'mobile_read_photo_content_' . $url_filter.'_'.$key_preview;
        ...
        else
        {
            ...
            $news_detail = cache("mobile_query-".$keyword_news, function () use ($keyword_news){
...
//            $news_detail['data_photos_visit'] = $this->photonews_detail_model->data_visit_photos($news_detail['news_id'], $keyword_paging);
            $data_visit_photos = cache("mobile_query-".$cacheKey."data_visit_photos", function () use($all_photos, $keyword_paging) {
...
//            $news_detail['total_photos'] = $this->photonews_detail_model->total_list_photos($news_detail['news_id']);
        ...
        {
            $prev_photos = cache("mobile_query-".$cacheKey."prev_photos_visit", function () use ($visit_page_no, $get_photo_list) {
                if($visit_page_no == 1)
                {
                    return "";
                }
        ...
        {
            $next_photos = cache("mobile_query-".$cacheKey."next_photos_visit", function () use ($visit_page_no, $get_photo_list) {
        ...
        {
            $tag_news = cache("mobile_query-". $cache_news_id ."-tag_news", function () use ($news_detail){
        ...
        if( !empty($news_detail['sponsor_tag']) )
        {
            ...
            $news_detail['reporter']            = $this->view('mobile/box/_box_brand', ['brand' => $news_detail['sponsor_tag'], 'TE' => $TE_2 ], true);
        ...
        foreach ($this->config['keyword_brand'] as $key => $value)
        {
            if ($key == $keyword_news )
            {
                ...
                $brand['brand_url']                 = '#';
                $news_detail['reporter']            = $this->view('mobile/box/_box_brand', ['brand' => $brand, 'TE' => $TE_2 ], true);
                $news_detail['reporter_style_date'] = 'style="margin-top: 28px;"';
                $news_detail['reporter_status']     = 'brand';
                $init_brand = TRUE;
            }
        ...
        //$news_detail['news_content'] = $this->add_te_link_in_content($news_detail['news_content'], $TE_1);
        //end kode track event

        // function crosslink
        $cross_view = '';
        $cross = cache('mobile_news_crosslink_'. $cache_news_id , function () use($news_detail) {
        ...
        $this->_mobile_render('mobile/photo/detail', $data);
    }

...
class TagsController extends CController {
    ...
    function index($tags_abjad = '', $tags_nama = '', $halaman = ''){
    	  ...
    	  $cacheKey = 'mobile_en_name_index_tag_'.$tags_abjad."_".$tags_nama."_".$halaman;
        $interval = WebCache::App()->get_config('cachetime_short');
        if ($ret = checkCache($cacheKey))
        {
        	return $ret;
        }
        $OG_URL = $this->config['www_url_en'].'tag/'.$tags_abjad."/".$tags_nama.'/'.$halaman;
        $TE_1 = 'Menu';
        $TE_2 = $tags_nama;
        $TE_3 = 'Tag pages';
        ...
        {
            //condition no index
            $CANONICAL = $this->config['www_url_en'].'tag/'.$tags_abjad."/".$tags_nama.'/';
        ...
        $data_meta_tags['url_full'] = $this->config['www_url_en'] . substr($tmp['news_url_full'], strlen($this->config['rel_url'])) ;
        ...
        [
            ...
            'og_url'            => $OG_URL,
            'og_image'          => $data_meta_tags['image_url_full'],
        ...
        $ret = $this->_mobile_render('mobile_en/tag/nametag', $data);
    ...
    public function get_tag_news_special_headline($tags_nama ,$keyword){
        ...
        $interval = WebCache::App()->get_config('cachetime_short');
        $cacheKey = 'mobile_en_name_index_tag_custom_'.$keyword;
        if ($ret = checkCache($cacheKey))
        {
            return unserialize($ret) ;
        }

        $get_news = cache('query_'.$cacheKey, function() use($keyword)
            {
    ...
    function sponsor_brand($tags_abjad = '', $tags_nama = '', $halaman = ''){
        ...
        $cacheKey = 'mobile_en_name_sponsor_brand_'.$tags_abjad."_".$tags_nama."_".$halaman;
        $interval = WebCache::App()->get_config('cachetime_short');
        if ($ret = checkCache($cacheKey))
        {
            return $ret;
        ...
        if ( $halaman == 1 || empty($halaman) )
        {
            ...
            $news = cache("mobile_en_query_".$cacheKey."_".$per_pages."_".$offset, function() use($news, $per_pages, $offset) {
        ...
        else
        {
            ...
            $news = cache("mobile_en_query_".$cacheKey."_".$per_pages."_".$offset, function() use($per_pages, $offset, $news) {
                return $news->with('news_tag_list')
                        ->skip($offset)
                        ->take($per_pages)
                        ->where('news_date_publish', '<=', date('Y-m-d H:i:s'))
            ...
            if (strpos($halaman, '-') !== false)
            {
                ...
                $promote_article = cache("mobile_en_query_news_sponsor_brand_".$keyword, function() use($keyword) {
        ...
        $data_meta_tags['url_full']       = $this->config['www_url_en_en'] . substr($tmp['news_url_full'], strlen($this->config['rel_url'])) ;
        $data_meta_tags['image_url_full'] = $tmp['news_image_url_full'];
        $data_meta_tags['news_synopsis']  = $tmp['news_synopsis'];
        $data_meta_tags['news_entry']     = $tmp['news_entry'];
        //EOF meta tags
        ...
        $ret = $this->_mobile_render('mobile_en/tag/tagbrand', $data);

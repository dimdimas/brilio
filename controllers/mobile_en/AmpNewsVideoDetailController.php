...
class AmpNewsVideoDetailController extends CController {
    ...
    function index($category = '', $slug = '', $paging = '') {
        ...
        $cacheKey = 'amp_en_mobile_video_news_detail_' . $slug;
        ...
        else
        {
            ...
            if ($_video)
            {
               ...
               $cross = cache('amp_en_video_query_news_crosslink_'.$news_detail['news_id'], function () use($news_detail) {
               ...
               if ($news_detail['has_paging'] == 1){
                  $paging_ = cache('amp_en_video_query_berita_paging_'. $news_detail['news_id'] , function() use($news_detail) {
              ...
              if ($news_detail['has_paging'] == 1)
              {
                  ...
                  if ($status_order== 0)
                  {
                      ...
                      $paging_list = cache('amp_en_video_query_nav_top_paging_'.$keyword_paging. '_' .$news_detail['news_id'], function() use($keyword_paging, $news_detail, $limit, $offset)
                      {
   ...
   $interval   = WebCache::App()->get_config('cachetime_default');
   $cacheKey = 'amp_en_video_mobile_related_' . MD5($news_detail['news_id']);
   ...
   $_v = cache('query_en_amp_mobile_more_stories_desktop_'.$news_detail['news_id'], function() use( $news_detail, $total_related) {

...
class NewsDetailController extends CController {
     ...
     function index($category = '', $slug = '', $url_paging_news = '')
     {
        ...
        {
            $cacheKey = 'mobile_en_read_news_content_' . $slug.'_'.$key_preview;
        ...
        {
            $cacheKey = 'mobile_en_read_news_content_' . $slug. '-' .$url_paging_news.'_'.$key_preview;
       ...
       //FUNCTION SPONSOR TAG
        ...
        if (isset($news_detail['news_crosslink']) && !empty($news_detail['news_crosslink'])) {
          	...
          	foreach ($cross as $key=>$value)
          	{
             	...
             	{
                	$cross = $this->view('mobile_en/detail/_crosslink',['cross' => $value],TRUE);
        ...
        $data_breadcumb['category_url']     = strtolower($news_detail['news_category_url']);
          ...
          [
                ...
                'news_related'  => $this->view('mobile_en/detail/_news_related', ['TE_2' => $TE_2, 'related_news_list' => array_slice($news_detail['related_news_list'],0,3)], TRUE),
                'popular'       => $this->_trending(6, $TE_2),
                'split_data'    => $news_detail['news_split_data'],
                '_list_categories_bottom' => $this->view('mobile_en/box/_menu_bottom', ['_list' => $this->mobile_menu_bottom($TE_2) ], true),
          ...
          $ret = $this->_mobile_render('mobile_en/detail/m_news_detail', $data);
    ...
    function more_stories($news_detail, $TE){
    	$interval   = WebCache::App()->get_config('cachetime_default');
      	$cacheKey = 'mobile_en_more_stories_' . MD5($news_detail['news_id']);

...
class AmpPhotoDetailController extends CController {
    ...
    function index($url_keyword_news = '', $url_paging_news = '')
    {
        ...
        $cacheKey   = 'amp_en_photo_mobile_read_photo_content-' . $url_filter;
        $TE_1 = 'Menu';
        $TE_2 = 'Detail Foto';
...
//            $news_detail = $this->news_model->get_read_photo($keyword_news);
            $news_detail = cache("amp_en_photo_mobile_query-".$keyword_news, function () use ($keyword_news){
...
//            $news_detail['data_photos_visit'] = $this->photonews_detail_model->data_visit_photos($news_detail['news_id'], $keyword_paging);
            $data_visit_photos = cache("amp_en_photo_mobile_query-".$cacheKey."data_visit_photos", function () use($all_photos, $keyword_paging) {
...
//            $news_detail['total_photos'] = $this->photonews_detail_model->total_list_photos($news_detail['news_id']);
        ...
        {
            $prev_photos = cache("amp_en_photo_mobile_query-".$cacheKey."prev_photos_visit", function () use ($visit_page_no, $get_photo_list) {
                if($visit_page_no == 1)
                {
                    return "";
                }
...
//            $news_detail['next_photos_visit'] = $this->photonews_detail_model->next_visit_photos($news_detail['news_id'], $news_detail['data_photos_visit']['photonews_id']);
            $next_photos = cache("amp_en_photo_mobile_query-".$cacheKey."next_photos_visit", function () use ($visit_page_no, $get_photo_list) {
...
//            $news_detail['tag_news'] = $this->tag_news_model->get_tags_news($news_detail['news_id']);
            $tag_news = cache("amp_en_photo_mobile_query-". $cache_news_id ."-tag_news", function () use ($news_detail){
        ...
        // function crosslink
        $cross_view = '';
        $cross = cache('amp_en_photo_mobile_news_crosslink_'. $cache_news_id , function () use($news_detail) {
    ...
    function get_related($news_detail, $TE){
      ...
      $_v = cache("mobile_query_en__related_per_tag", function () use($date_publish, $total_related) {

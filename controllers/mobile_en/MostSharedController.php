...
class MostSharedController extends CController {
    ...
    function index($thn = '', $bln = '', $tgl = '') {
        ...
        $cacheKey = 'mobile_en_index_most_shared_'.$tanggal.'-'.$thn;
        if ($ret  = checkCache($cacheKey)) return $ret;

        $tanngal_sekarang = date("Y-m-d");
        //meta tag

        $most_shared = $this->most_shared_news($thn, $bln, $tgl);

        $og_image            = '';
        $og_image_secure     = '';
        $og_url              = '';
        if(!isset($most_shared['ERROR']))
        {
            // $popular_news    = $popular_news[0];
            $og_url          = $most_shared[0]['OG_URL'];
            //$sosmed          = $this->sosmed($popular_news['OG_URL']);
            $og_image        = 'http://cdn.klimg.com/newshub.id/'. substr($most_shared[0]['NEWS_IMAGES'], strlen($this->config['klimg_url']));
            $og_image_secure = 'https://cdn.klimg.com/newshub.id/'. substr($most_shared[0]['NEWS_IMAGES'], strlen($this->config['klimg_url']));

            //set news headline and list;
            $headline = $most_shared[0];
            unset($most_shared[0]);
            $news_list = $most_shared;
        }
        else
        {
        ...
        $meta = array(
            ...
            'meta_keywords'     => isset($most_shared['NEWS_SYNOPSIS'][0]) ? str_replace(' ', ', ', $most_shared['NEWS_SYNOPSIS'][0]) : 'Kisah, dan, cerita, kehidupan, gaya, hidup, renungan, inspirasi, dan, ilmu, pengetahuan, paling, banyak, dikomentari',
            'og_url'            => $og_url,
            'og_image'          => $og_image,
        ...
        $ret = $this->_mobile_render('mobile_en/most-shared/index_most_shared', $data);
    ...
    function menu_type_date($thn, $bln, $tgl) {
        ...
        $cacheKey = 'mobile_en_menu_type_date_most_shared_'.$tanggal.'-'.$thn;
    ...
    function menu_per_date($thn, $bln, $tgl) {
        ...
        $cacheKey = 'mobile_en_menu_per_date_most_shared_'.$tanggal.'-'.$thn;
        if ($ret = checkCache($cacheKey))
            return $ret;
    ...
    function most_shared_news($thn, $bln, $tgl) {

        $interval = WebCache::App()->get_config('cachetime_short');
        $cacheKey = 'mobile_en_most_shared_news_'.date("Y-m-d").'-'.$thn;
        ...
        $news = cache("mobile_en_query_shared".$cacheKey, function () use ($start_date, $end_date)
        {

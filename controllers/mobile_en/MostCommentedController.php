...
class MostCommentedController extends CController {
    ...
    function index($thn = '', $bln = '', $tgl = '') {
        ...
        $cacheKey = 'mobile_en_index_most_commented_'.$tanggal.'-'.$thn;
        if ($ret  = checkCache($cacheKey)) return $ret;

        $tanngal_sekarang = date("Y-m-d");
        //meta tag

        $most_commented_news = $this->most_commented($thn, $bln, $tgl);
        // echoPre($most_commented_news);
        $og_image            = '';
        $og_image_secure     = '';
        $og_url              = '';
        if(!isset($most_commented_news['ERROR']))
        {
            $og_url          = $most_commented_news[0]['OG_URL'];
            //$sosmed          = $this->sosmed($popular_news['OG_URL']);
            $og_image        = 'http://cdn.klimg.com/newshub.id/'. substr($most_commented_news[0]['NEWS_IMAGES'], strlen($this->config['klimg_url']));
            $og_image_secure = 'https://cdn.klimg.com/newshub.id/'. substr($most_commented_news[0]['NEWS_IMAGES'], strlen($this->config['klimg_url']));

            //set news headline and list;
            $headline = reset($most_commented_news);
            unset($most_commented_news[0]);
            $news_list = $most_commented_news;
        }
        else
        {
        ...
        $meta = array(
            ...
            'meta_keywords'     => isset($most_commented_news['NEWS_SYNOPSIS'][0]) ? str_replace(' ', ', ', $most_commented_news['NEWS_SYNOPSIS'][0]) : 'Kisah, dan, cerita, kehidupan, gaya, hidup, renungan, inspirasi, dan, ilmu, pengetahuan, paling, banyak, dikomentari',
            'og_url'            => $og_url,
            'og_image'          => $og_image,
        ...
        $data = array(
            'full_url'          => $url,
            'meta'              => $meta,
            'title'             => 'Most Commented Articles - Brilio.net',
            'TE_2'              => $TE_2,
        ...
        $ret = $this->_mobile_render('mobile_en/most-commented/index_most_commented', $data);
    ...
    function breadcrumb($thn, $bln, $tgl) {
        ...
        $cacheKey = 'breadcrumb_most_commented_mobile-'.$tanggal.'-'.$thn;
        if ($ret = checkCache($cacheKey)) return $ret;
    ...
    function menu_type_date($thn, $bln, $tgl) {
        ...
        $cacheKey = 'mobile_en_menu_type_date_most_commented_'.$tanggal.'-'.$thn;
    ...
    function menu_per_date($thn, $bln, $tgl) {
        ...
        $cacheKey = 'mobile_en_menu_per_date_most_commented_'.$tanggal.'-'.$thn;
    ...
    function most_commented($thn, $bln, $tgl) {

        $interval = WebCache::App()->get_config('cachetime_short');
        $cacheKey = 'mobile_en_most_commented_'.date("Y-m-d").'-'.$thn;
        ...
        $news = cache("mobile_en_query_commented_".$cacheKey, function () use ($start_date, $end_date)
        {

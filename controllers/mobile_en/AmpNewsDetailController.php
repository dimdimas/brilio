...
class AmpNewsDetailController extends CController {
     ...
     function index($category = '', $slug = '', $url_paging_news = '')
     {
        ...
        $cacheKey = 'amp_mobile_en_read_news_content_' . $slug;
        }
        else{
            $cacheKey = 'amp_mobile_en_read_news_content_' . $slug. '-' .$url_paging_news;
        }
        ...
        else
        {
          ...
          $tags = cache('amp_en_query_news_tags_'.$news_detail['news_id'], function () use($news_detail) {
          ...
          //function crosslink
           $cross = cache('amp_en_query_news_crosslink_'.$news_detail['news_id'], function () use($news_detail) {
          ...
          if ($news_detail['has_paging'] == 1)
          {
            $TE = 'Detail paging';
            $TE_2 = 'Detail paging';
            $paging_ = cache('amp_en_query_berita_paging_'. $news_detail['news_id'] , function() use($news_detail) {
          ...
          if ($status_order== 0)
          {
               ...
               $paging_list = cache('amp_en_query_nav_top_paging_'.$keyword_paging. '_' .$news_detail['news_id'], function() use($keyword_paging, $news_detail, $limit, $offset)
               {
        ...
        if( !empty($news_detail['sponsor_tag']) )
        {
            ...
            $news_detail['reporter']['name']   = $this->view('mobile_en/box/amp_box_brand', [ 'brand' => $news_detail['sponsor_tag'], 'TE' => $TE_2], true);
        ...
        foreach ($this->config['keyword_brand'] as $key => $value)
        {
            if ($key == $keyword_news )
            {
                ...
                $brand['brand_url']                 = '#';
                $news_detail['reporter']['name']    = $this->view('mobile_en/box/amp_box_brand', ['brand' => $brand, 'TE' => $TE_2 ], true);
                $news_detail['reporter_style_date'] = 'style="margin-top: 28px;"';
                $news_detail['reporter_status']     = 'brand';
                $init_brand = TRUE;
            }
        ...
        if (isset($news_detail['news_crosslink']) && !empty($news_detail['news_crosslink'])) {
          ...
          foreach ($cross as $key=>$value)
          {
             ...
             {
                $cross = $this->view('mobile_en/detail/amp_crosslink',['cross' => $value],TRUE);
        ...
        $data_breadcumb['category_url']     = strtolower($news_detail['news_category_url']);
          ...
          [
                ...
                'TE_2'          => $TE_2,
                'meta'          => $meta,
                'box_announcer' => $this->view('mobile_en/box/amp_announcer_banner', [], TRUE),
                'breadcrumb'    => $data_breadcumb,
                'news_data'     => $this->_amp_validation($news_detail),
                'news_related'  => $this->view('mobile_en/detail/amp_news_related', ['related_news_list' => $news_detail['related_news_list']], TRUE),
                'popular'       => $this->_trending(6, $TE_2),
                'split_data'    => $news_detail['news_split_data'],
                'today_tags'    => $this->view('mobile_en/box/amp_tags_bottom', ['list_tags_bottom' => $this->_today_tags(0, 10, 0, $TE_2) ], true),
                'collect_email' => $this->view('mobile_en/box/_collect_email', [], true),
                'video'         => $this->view('mobile_en/box/amp_promote_video', ['promote_video' => $promote_video ], TRUE),
          ];
     ...
     function get_news_related($news_detail, $tags){
      $interval   = WebCache::App()->get_config('cachetime_default');
      $cacheKey = 'amp_en_mobile_desktop_related_' . MD5($news_detail['news_id']);
      ...
      if ($ret = checkCache($cacheKey))
        return unserialize($ret) ;
          ...
          // if (empty($tags)) {
          //     $related_news_list = '';
          //     return $this->view('mobile_en/news/_news_related', ['related_news_list' => $related_news_list], TRUE);
          // }

          $relstory = cache('amp_en_query_related_story_'.$news_detail['news_id'], function() use($tags, $news_detail, $temp_news_related, $news_index, $total_related) {

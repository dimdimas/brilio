...
class PopularController extends CController {
    ...
    function index($thn = '', $bln = '', $tgl = '') {
        ...
        $cacheKey = 'mobile_en_index_popular_'.$tanggal.'-'.$thn;
        if ($ret  = checkCache($cacheKey)) return $ret;

        $tanngal_sekarang = date("Y-m-d");
        //meta tag

        $popular_news = $this->popular($thn, $bln, $tgl);

        $og_image            = '';
        $og_image_secure     = '';
        $og_url              = '';
        if(!isset($popular_news['ERROR']))
        {
            $og_url          = $popular_news[0]['OG_URL'];
            $og_image        = 'http://cdn.klimg.com/newshub.id/'. substr($popular_news[0]['NEWS_IMAGES'], strlen($this->config['klimg_url']));
            $og_image_secure = 'https://cdn.klimg.com/newshub.id/'. substr($popular_news[0]['NEWS_IMAGES'], strlen($this->config['klimg_url']));

            //set news headline and list;
            $headline = $popular_news[0];
            unset($popular_news[0]);
        ...
        $meta = array(
            ...
            'meta_keywords'     => isset($popular_news['NEWS_SYNOPSIS'][0]) ? str_replace(' ', ', ', $popular_news['NEWS_SYNOPSIS'][0]) : 'Kisah, dan, cerita, kehidupan, gaya, hidup, renungan, inspirasi, dan, ilmu, pengetahuan, paling, banyak, dikomentari',
            'og_url'            => $og_url,
            'og_image'          => $og_image,
        ...
        $data = array(
            'full_url'          => $url,
            'meta'              => $meta,
            'title'             => 'Most Popular Articles - Brilio.net',
            'TE_2'              => $TE_2,
        ...
        $ret =  $this->_mobile_render('mobile_en/popular/index_popular', $data);
    ...
    function menu_type_date($thn, $bln, $tgl) {
        ...
        $cacheKey = 'menu_type_date_popular_mobile_en_'.$tanggal.'-'.$thn;
        if ($ret = checkCache($cacheKey)) return $ret;
    ...
    function menu_per_date($thn, $bln, $tgl) {
        ...
        $cacheKey = 'menu_per_date_popular_mobile_en_'.$tanggal.'-'.$thn;
        if ($ret = checkCache($cacheKey)) return $ret;
    ...
    function popular($thn, $bln, $tgl) {

        $interval = WebCache::App()->get_config('cachetime_short');
        $cacheKey = 'mobile_en_popular_'.date("Y-m-d").'-'.$thn;
        ...
        $news = cache("query_mobile_en_popular".$cacheKey, function () use ($start_date, $end_date)
        {
            ...
            foreach ($news as $data)
            {
                ...
                if($no == 1)
                {
                    ...
                    if ( is_numeric($data['news_entry']) ) {
                      $data_news['NEWS_ENTRY']  = $this->lib_date->mobile_waktu( date('Y-m-d H:i:s', $data['news_entry']));
                    } else {
    ...
    //SOSMET
    function sosmed($url = ''){
        $cacheKey = 'mobile_en_sosmed_popular_'.  md5($url);
        if ($ret = checkCache($cacheKey))
        {
            return $ret;
        }
        ...
        $ret = $this->view('mobile_en/box/_sosmed', $sosmed_data, true);
        $interval = WebCache::App()->get_config('cachetime_short');
        setCache($cacheKey, $ret, $interval);
        return $ret;
    }

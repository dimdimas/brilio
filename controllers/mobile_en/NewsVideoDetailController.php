...
class NewsVideoDetailController extends CController {
    ...
    function index($category = '', $slug = '', $paging = '') {
        ...
        $cacheKey = 'mobile_en_video_news_detail_' . $slug.'_'.$key_preview;
        ...
        if (!empty($mongo_data))
        {
            ...
            {
               $news_detail['related_news_list'] = $this->get_news_related($news_detail, $news_detail['news_tag_list'], $TE_2);
            }
        ...
        else
        {
            ...
            if ( !empty($_video[0]) )
            {
                ...
                //GET TAG
                $tags = cache('query_mobile_en_news_tags_'.$news_detail['news_id'], function () use($news_detail) {
               ...
               $cross = cache('query_mobile_en_news_crosslink_'.$news_detail['news_id'], function () use($news_detail) {
               ...
               if ($news_detail['has_paging'] == 1){
                  $paging_ = cache('query_mobile_en_berita_paging_'. $news_detail['news_id'] , function() use($news_detail) {
              ...
              if ($news_detail['has_paging'] == 1)
              {
                  ...
                  if ($status_order== 0)
                  {
                      ...
                      $paging_list = cache('query_mobile_en_nav_top_paging_'.$keyword_paging. '_' .$news_detail['news_id'], function() use($keyword_paging, $news_detail, $limit, $offset)
                      {
        ...
        if( !empty($news_detail['sponsor_tag']) )
        {
            ...
            $news_detail['reporter']['name']   = $this->view('mobile_en/box/_box_brand', [ 'brand' => $news_detail['sponsor_tag'], 'TE' => $TE_2], true);
        ...
        foreach ($this->config['keyword_brand'] as $key => $value)
        {
            if ($key == $keyword_news )
            {
                ...
                $brand['brand_url']                 = '#';
                $news_detail['reporter']['name']    = $this->view('mobile_en/box/_box_brand', ['brand' => $brand, 'TE' => $TE_2 ], true);
                $news_detail['reporter_style_date'] = 'style="margin-top: 28px;"';
                $news_detail['reporter_status']     = 'brand';
                $init_brand = TRUE;
            }
      ...
      if (isset($news_detail['news_crosslink']) && !empty($news_detail['news_crosslink'])) {
            ...
            foreach ($cross as $key=>$value)
            {
                ...
                {
                    $cross = $this->view('mobile_en/video/_crosslink',['cross' => $value],TRUE);
      ...
      $data_breadcumb['category_url']     = strtolower($news_detail['news_category_url']);
        ...
        [
            ...
            'news_related'  => $this->view('mobile_en/detail/_news_related', ['related_news_list' => array_slice($news_detail['related_news_list'], 0, 3)], TRUE),
            ...
            'collect_email' => $this->view('mobile_en/box/_collect_email', [], true),
        ...
        $ret = $this->_mobile_render('mobile_en/video/newsvideodetail', $data);
        $interval = WebCache::App()->get_config('cachetime_default');
   ...
   function more_stories($news_detail, $TE){
   	...
   	$interval   = WebCache::App()->get_config('cachetime_default');
   	$cacheKey = 'mobile_en_video_more_stories_' . MD5($news_detail['news_id']);
    ...
    $_v = cache('query_mobile_en_more_stories_'.$news_detail['news_id'], function() use( $news_detail, $total_related) {
